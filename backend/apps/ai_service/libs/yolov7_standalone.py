...
def load_yolov7_model(weights_path, device='cpu'):
    ...
    优先直接使用 checkpoint 中的模型对象（避免重建时通道不匹配），
    仅在无法直接使用时才从 yaml 配置重建。
    ...
    Returns:
        model: 加载了权重的模型（可能是原始训练类或 YOLOModel）
        names: 类别名称列表
        stride: 模型步长
    """
    ...
    if hasattr(model_state, 'yaml'):
        ...
        raise ValueError("无法加载模型：检查点中未找到模型配置")

    # 提取类别名称（优先级：checkpoint['names'] > model_state.names > 默认）
    names = [f'class{i}' for i in range(nc)]
    if 'names' in checkpoint and checkpoint['names'] is not None:
        raw = checkpoint['names']
    elif hasattr(model_state, 'names') and model_state.names is not None:
        raw = model_state.names
    else:
        raw = None

    if raw is not None:
        if isinstance(raw, dict):
            names = [raw.get(i, raw.get(str(i), f'class{i}')) for i in range(max(raw.keys()) + 1)]
            if len(names) < nc:
                names += [f'class{i}' for i in range(len(names), nc)]
        elif isinstance(raw, (list, tuple)):
            names = list(raw)[:nc] if len(raw) >= nc else list(raw) + [f'class{i}' for i in range(len(raw), nc)]
    ...
    # 尝试直接使用原始模型对象（已含正确权重，无需 load_state_dict）
    if hasattr(model_state, 'forward'):
        model = model_state
    else:
        # 回退：从 yaml 重建
        model = YOLOModel(cfg, nc=nc)
        state_dict = model_state.get('state_dict', model_state) if isinstance(model_state, dict) else model_state.state_dict()
        model.load_state_dict(state_dict, strict=False)
