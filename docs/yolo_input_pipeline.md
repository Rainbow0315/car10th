...
巡航任务启动时也会走同一条 monitor 链路：

```text
App 巡航任务 start
  -> web_api /api/patrol/tasks/{task_code}/start
  -> patrol_service 发布航点导航目标
  -> patrol_service 启动 inspection_monitor
  -> inspection_monitor 周期调用 ai_service detect-ros-image
  -> enabled_models=["unified"]
```

如果任务的 `detection_config.enabled` 显式为 `false`，巡航只跑导航，不启动 YOLO。没有配置时默认启动 `unified` 巡航模型。
