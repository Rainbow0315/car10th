...
- 小车部署：不从 `dev` 触发，也不在 GitHub Actions 里做；只允许从已合并的 `main` 由部署机执行

## 先说明一个关键点

GitHub 无法做到“开发者先 push 到 `dev` 之前就先跑 CI 再决定让不让 push”。

另一个边界也要说清楚：`dev` 只用于开发集成和 CI 检查，不承担小车部署职责。小车端不能稳定访问 GitHub，因此本项目不在 `dev` 上配置任何小车 CD；稳定版本合并到 `main` 后，再由 Windows 部署机拉取 `main`，通过 SSH/SCP 部署到小车。
...
5. 需要真机部署时，只从 `main` 执行小车端部署脚本

也就是说，远端 `dev` 分支只保留 CI 质量门禁；如果后续看到任何从 `dev` 自动部署小车的 workflow，都应该删除或禁用。
