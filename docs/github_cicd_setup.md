...
- 小车部署/CD：不从 `dev` 触发，只允许从已合并的 `main` 由部署机执行

## 先说明一个关键点

GitHub 无法做到“开发者先 push 到 `dev` 之前就先跑 CI 再决定让不让 push”。

另一个边界也要说清楚：`dev` 只用于开发集成和 CI 检查，不承担小车部署职责。小车端不能稳定访问 GitHub，因此本项目的小车 CD 不是 GitHub Actions 直接推到小车，而是 Windows 部署机拉取 `main` 后，通过 SSH/SCP 部署到小车。
